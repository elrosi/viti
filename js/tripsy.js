 ...
 * Result for column K
 * Formula: K = V3 /V4 * H
 * @param H {Object} Geöffnete (benützte) Düsen
 * @param J {Object} Einsparung bei Recyclinggeräten [%]
 * @param {string} ID | ID of the spreadsheet
 * @return {number}
 */
function LEAF_WALL_WATER_VOLUME(H,J,ID) {
 ...
 * @return {number}
 */
function LEAF_WALL_PRODUCT_PER_SPRAING(L,M,K,ID) {
    ...
    Kv = K.v;
    if (typeof Kv == 'string' && Iv.charAt(0) != "=") {
...
jQuery(document).ready(function() {
    ...
    var submit = jQuery( "#calculation-form" ).submit(function(event ) {
        ...
        if (jQuery('#'+calculator).length == 0) {
            ...
            if (cal_type == "Classic") {
                ...
                optionsLeafWall.data = [['', '', null, null, distanceBetweenRows, 0, 0, 0, 0, 0, '=LEAF_WALL_WATER_VOLUME(H$,"#'+calculator+'")', '=LEAF_WALL_DOES_RATE_PER_HA(C$, E$, G$, J$, "#'+calculator+'")', '=LEAF_WALL_PRODUCT_PER_FARM(L$, I$, G$, E$, "#'+calculator+'")','=LEAF_WALL_PRODUCT_PER_SPRAING(L$,M$,K$,"#'+calculator+'")', null]];
    ...
    jQuery(document).on('submit', '#row-treatment-lwa', function (e) {
        ...
        var rowOptionLeaftWall = [date, product, dose_rate, unit, distance, dose_max, height, opened_nozzles,
            ...
            '=LEAF_WALL_PRODUCT_PER_SPRAING(L$, M$, I$,"'+calTable+'")', '=LEAF_WALL_PRODUCT_PER_SPRAING(L$, M$, K$, "'+calTable+'")', null];
