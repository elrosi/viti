    ...
    var methods = {
        ...
        init: function (options) {
            ...
            var defaults = {
                ...
                // message text
                messages: {
                    notfound: "This field is required.",
        ...
        createTable: function () {
            ...
            if (options.tableClass) {
                $(table).prop('class',  options.tableClass + ' ' + 'jexcel bossanova-ui' );
            } else {
            ...
            if (options.growthStage) {
                $(table).attr('data-growth-stage', options.growthStage);
            }
            ...
            for (var i = 0; i < options.colHeaders.length; i++) {
                ...
                var title = options.colHeadersTitle[i] || options.colHeaders[i];

                // Header
                var header = options.colHeaders[i] || $.fn.jexcel('getColumnName', i);

                // Create HTML row
                tr += '<td id="col-' + i + '" width="' + width + '" align="' + align + '" title="' + title + '" class="tooltipster ' + className + '" ' + display + '>' + header + '</td>';
                    ...
                    if ($.fn.jexcel.current) {
                        ...
                        if ($.fn.jexcel.selectedCell) {
                            ...
                            if (e.which == 37) {
                                ...
                                // Delete (erase the cell if delete is enabled with the delete key)
                                if (options.eraseCell == true) {
        ...
        setData: function (data, ignoreSpare) {
            ...
            for (j = 0; j < $.fn.jexcel.defaults[id].data.length; j++) {
                ...
                $(tr).prop('id', 'row-' + parseInt(j)).data('row', parseInt(j + 1));


                // Index column
                $(tr).append('<td class="jexcel_label">' + parseInt(j + 1) + '</td>');

                var action = '<a class="tripsy-delete-link" onclick="$(\'#'+ id + '\').jexcel(\'deleteRow\', ' + j + '); $(\'#' + id + '\').jexcel(\'updateSelection\');"><span class="tripsy-delete-icon"></span></a>';
                var groupLength = $.fn.jexcel.defaults[id].colHeaders.length;

                // Data columns
                for (i = 0; i < groupLength; i++) {

                    // Do something if is the last iteration of the array
                    if (options.actionMenu === true) {
                        if ((i + 1) == (groupLength)) {
                            $.fn.jexcel.defaults[id].data[j][i] = action;
                        }
                    }
                ...
                if (options.columns[position[0]].editor) {
                    ...
                    if (options.columns[position[0]].type == 'checkbox' || options.columns[position[0]].type == 'hidden') {
                        ...
                        showResult = function (data, str) {
                            ...
                            if (!$(result).html()) {
                                $(result).html('<div style="padding:6px;">' + options.messages.notresult + '</div>');
                            }
                            $(result).css('display', '');
                        }
                        ...
                        showResult = function (data, str) {
                            ...
                            if (!$(result).html()) {
                                $(result).html('<div style="padding:6px;">' + options.messages.notfound + '</div>');
                            }
        ...
        getValue: function (cell) {
            ...
            if ($(cell).length) {
                ...
                if (options.columns[position[0]].editor) {
                    ...
                    } else if (options.columns[position[0]].type == 'formula') {
                        value = $(cell).text();
                    } else if (options.columns[position[0]].type == 'action') {
                        value = null;
                    } else {
                        // Get default value
            ...
            if ($(v.cell).hasClass('readonly') == true && force == false) {
                ...
                if (options.columns[position[0]].editor) {
                    ...
                    if (options.columns[position[0]].type == 'checkbox') {
                        ...
                        // Is this value a formula
                        if (value.substr(0, 1) == '=') {
                            $(this).jexcel('updateFormulas', v.col + '-' + v.row, value, v.row);
         ...
         * @return void
         */
        insertRow: function (mixed, rowNumber, records) {
            // Main
            var main = $(this);
            ...
            if (options.allowInsertRow == true) {
                ...
                if ($.fn.jexcel.ignoreEvents != true) {
                    if (typeof (options.oninsertrow) == 'function') {
                        options.oninsertrow($(this), rowNumber, records);
                    }
                }
         ...
         */
        updateFormulas: function (columnId, value, rowNumber) {

            if (typeof excelFormulaUtilities == 'object') {
                // Id
                var id = $(this).prop('id');

                var position = columnId.split("-");

                if (typeof rowNumber === 'undefined' || rowNumber === null || rowNumber === 0) {
                    rowNumber = parseInt(position[1]) + 1;
                }

                // Replace the variable name with the current line number
                value = value.replace(/[$|[\]\\]/g, rowNumber); // $& means the whole matched string
            ...
            if (typeof excelFormulaUtilities == 'object') {
                ...
                if ($.fn.jexcel.defaults[id].formula[column]) {
                    ...
                    $.each($.fn.jexcel.defaults[id].formula[column], function (k, v) {
                        ...
                        // Replace the variable name with the current line number
                        formula = formula.replace(/[$|[\]\\]/g, parseInt(cellId[1]) + 1);
                        ...
                        if (formula.substr(0, 1) != '=') {
                            ...
                            $(main).jexcel('executeFormula', v, parseInt(cellId[1]) + 1);
                        }

                        // Recursive formula
                        $(main).jexcel('formula', v, parseInt(cellId[1]) + 1);
                    });
                }
            } else {
         ...
         */
        executeFormula: function (cellId, rowNumber) {
            var main = $(this);
            ...
            // Replace the variable name with the current line number
            formula = formula.replace(/[$|[\]\\]/g, rowNumber);
            ...
            if (typeof excelFormulaUtilities == 'object') {
                ...
                if (columnIds.length > 0) {
                    for (var i = 0; i < columnIds.length; i++) {
                        pozitions[i] = columnIds[i].split('-');
                        var rowNumber = parseInt(pozitions[i][1]) + 1;
                        $(this).jexcel('formula', columnIds[i], rowNumber);
                    }
                }
            }
        },
         ...
         * Create cell
         */
        createCell: function (i, j, value = null) {
        ...
        updateTableReferences: function (referenceCol, referenceRow) {
            ...
            for (var j = 0; j < $.fn.jexcel.defaults[id].data.length; j++) {
                for (var i = 0; i < $.fn.jexcel.defaults[id].data[j].length; i++) {
                    ...
                    if (val.substr(0, 1) == '=') {
                        $(main).jexcel('updateFormulas', i + '-' + j, val, referenceRow);
                    }
                }
            }
